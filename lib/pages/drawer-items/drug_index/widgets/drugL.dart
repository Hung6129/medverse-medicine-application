...
class DrugL extends StatefulWidget {
  const DrugL({Key key}) : super(key: key);

  @override
  State<DrugL> createState() => _DrugLState();
}

class _DrugLState extends State<DrugL> {
  bool isLoading = false;
  int count = 0;
  List _listL;

  /// Method call all data from SQLite
  Future<List<DrugModel>> getAllDrugsL(int count) async {
    if (count < 20) {
      _listL = await DataDao().getDrugL(count);
    } else {
      var _newListL = await DataDao().getDrugL(count);
      _listL.addAll(_newListL);
    }
    return _listL;
  }
