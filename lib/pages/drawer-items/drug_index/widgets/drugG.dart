...
class DrugG extends StatefulWidget {
  const DrugG({Key key}) : super(key: key);

  @override
  State<DrugG> createState() => _DrugGState();
}

class _DrugGState extends State<DrugG> {
  bool isLoading = false;
  int count = 0;
  List _listG;

  /// Method call all data from SQLite
  Future<List<DrugModel>> getAllDrugsG(int count) async {
    if (count < 20) {
      _listG = await DataDao().getDrugG(count);
    } else {
      var _newListG = await DataDao().getDrugG(count);
      _listG.addAll(_newListG);
    }
    return _listG;
  }
