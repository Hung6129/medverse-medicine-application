...
class DrugE extends StatefulWidget {
  const DrugE({Key key}) : super(key: key);

  @override
  State<DrugE> createState() => _DrugEState();
}

class _DrugEState extends State<DrugE> {
  bool isLoading = false;
  int count = 0;
  List _listE;

  /// Method call all data from SQLite
  Future<List<DrugModel>> getAllDrugsE(int count) async {
    if (count < 20) {
      _listE = await DataDao().getDrugE(count);
    } else {
      var _newListE = await DataDao().getDrugE(count);
      _listE.addAll(_newListE);
    }
    return _listE;
  }
