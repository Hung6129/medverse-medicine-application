...
class DrugN extends StatefulWidget {
  const DrugN({Key key}) : super(key: key);

  @override
  State<DrugN> createState() => _DrugNState();
}

class _DrugNState extends State<DrugN> {
  bool isLoading = false;
  int count = 0;
  List _listN;

  /// Method call all data from SQLite
  Future<List<DrugModel>> getAllDrugsN(int count) async {
    if (count < 20) {
      _listN = await DataDao().getDrugN(count);
    } else {
      var _newListN = await DataDao().getDrugN(count);
      _listN.addAll(_newListN);
    }
    return _listN;
  }
