...
class DrugW extends StatefulWidget {
  const DrugW({Key key}) : super(key: key);

  @override
  State<DrugW> createState() => _DrugWState();
}

class _DrugWState extends State<DrugW> {
  bool isLoading = false;
  int count = 0;
  List _listW;

  /// Method call all data from SQLite
  Future<List<DrugModel>> getAllDrugsW(int count) async {
    if (count < 20) {
      _listW = await DataDao().getDrugW(count);
    } else {
      var _newListW = await DataDao().getDrugW(count);
      _listW.addAll(_newListW);
    }
    return _listW;
  }
