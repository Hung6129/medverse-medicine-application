...
class DrugX extends StatefulWidget {
  const DrugX({Key key}) : super(key: key);

  @override
  State<DrugX> createState() => _DrugXState();
}

class _DrugXState extends State<DrugX> {
  bool isLoading = false;
  int count = 0;
  List _listX;

  /// Method call all data from SQLite
  Future<List<DrugModel>> getAllDrugsX(int count) async {
    if (count < 20) {
      _listX = await DataDao().getDrugX(count);
    } else {
      var _newListX = await DataDao().getDrugX(count);
      _listX.addAll(_newListX);
    }
    return _listX;
  }
