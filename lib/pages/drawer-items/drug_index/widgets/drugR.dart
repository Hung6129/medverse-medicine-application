...
class DrugR extends StatefulWidget {
  const DrugR({Key key}) : super(key: key);

  @override
  State<DrugR> createState() => _DrugRState();
}

class _DrugRState extends State<DrugR> {
  bool isLoading = false;
  int count = 0;
  List _listR;

  /// Method call all data from SQLite
  Future<List<DrugModel>> getAllDrugsR(int count) async {
    if (count < 20) {
      _listR = await DataDao().getDrugR(count);
    } else {
      var _newListR = await DataDao().getDrugR(count);
      _listR.addAll(_newListR);
    }
    return _listR;
  }
