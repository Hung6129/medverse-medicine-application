...
class DrugJ extends StatefulWidget {
  const DrugJ({Key key}) : super(key: key);

  @override
  State<DrugJ> createState() => _DrugJState();
}

class _DrugJState extends State<DrugJ> {
  bool isLoading = false;
  int count = 0;
  List _listJ;

  /// Method call all data from SQLite
  Future<List<DrugModel>> getAllDrugsJ(int count) async {
    if (count < 20) {
      _listJ = await DataDao().getDrugJ(count);
    } else {
      var _newListJ = await DataDao().getDrugJ(count);
      _listJ.addAll(_newListJ);
    }
    return _listJ;
  }
